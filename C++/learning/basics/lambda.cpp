...
int main() {
    ...
    int c = count_if(v1.begin(), v1.end(), [](int x ){return !(x%2);});
    cout << c << endl;

    // Copies the odd numbers
    // Will store x to evenVec any time when the return condition is true
    vector<int> evenVec;
    copy_if(v1.begin(), v1.end(), back_inserter(evenVec), [](int x){return (x%2);});
    ...
    vector<int> doubled;
    for_each(v2.begin(), v2.end(), [&](int x){doubled.push_back(x*2);});
